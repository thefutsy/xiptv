...
#define u_pointerNdc u_space.zw
...
vec3 shade(vec2 fieldUv, vec2 p, float t) {
  float y = fieldUv.y
    + sin(fieldUv.x * (3.0 + u_intensity * 9.0) + t * 0.8) * 0.08
...
void main() {
  ...
  if (u_cursorPresence > 0.001) {
    vec2 cursor = (0.5 * u_pointerNdc * u_resolution.xy)
      / min(u_resolution.x, u_resolution.y);
  ...
  vec2 fieldUv = p * min(u_resolution.x, u_resolution.y) / u_resolution.xy + 0.5;
  p *= u_scale;
  ...
  if (u_blur > 0.0) {
    ...
    col  = shade(fieldUv, p, u_time) * 0.36;
    col += shade(fieldUv + vec2(uvE.x, 0.0), p + vec2(pe, 0.0), u_time) * 0.16;
    col += shade(fieldUv - vec2(uvE.x, 0.0), p - vec2(pe, 0.0), u_time) * 0.16;
    col += shade(fieldUv + vec2(0.0, uvE.y), p + vec2(0.0, pe), u_time) * 0.16;
    col += shade(fieldUv - vec2(0.0, uvE.y), p - vec2(0.0, pe), u_time) * 0.16;
  } else {
    col = shade(fieldUv, p, u_time);
  }
...
const UNIFORMS = {
  // V3's greys, not the original teal: #0E0E10, #17171B, #26262D, #34343E.
  colors: [[0.0549,0.0549,0.0627],[0.0902,0.0902,0.1059],[0.1490,0.1490,0.1765],[0.2039,0.2039,0.2431],[0.2039,0.2039,0.2431],[0.2039,0.2039,0.2431],[0.2039,0.2039,0.2431],[0.2039,0.2039,0.2431]] as [number, number, number][],
  colorCount: 4,
  scale: 2.000,
  intensity: 0.540,
  paramA: 0.470,
